...
MPU9250_Result_t MPU9250_Init(MPU9250_t *MPU9250, MPU9250_Device_t dev, MPU9250_Accel_Scale_t accScale, MPU9250_Gyro_Scale_t gyroScale, MPU9250_Mag_Scale_t magScale)
{
	 ...
	 * @Fs = 8000 Hz
	 * @Gyro BW: 3600 Hz
	 * @Temperature BW: 4000 Hz
	 * @Note: Sensor fusion update rate cannot be higher than (1 / 0.17ms) = 5882 Hz
	 */
	writeByte(&hi2c1, MPU9250->I2C_Addr, CONFIG, 0x03);

	/* Sample Rate = Fs / (1 + SMPLRT_DIV)
	 * @Sample Rate = 8000 / (1 + 0) = 8000 Hz
	 * @Note: SMPLRT_DIV is only used for 1 kHz internal sampling.
	 */
	...
	data |= 0x08;		/* ACCEL_CONFIG_2[3:0] bits are set to 1000 */
	...
	writeByte(&hi2c1, MPU9250->I2C_Addr_Mag, CNTL, 0x00);
	HAL_Delay(10);
	/* MODE[3:0] Operation Mode: Continuous Measurement Mode [2]
	 * @Mode Configuration
	 * 		Mode[1] - 0010: 8 Hz
	 * 		Mode[2] - 0110: 100 Hz
	 * @BIT Output Bit Setting: 16-bit output
	 */
	writeByte(&hi2c1, MPU9250->I2C_Addr_Mag, CNTL, (uint8_t)((magScale << 4) | 0x06));
	HAL_Delay(10);

	/* Accelerometer Resolution Multiplicator: LSB / g 			*/
	switch (accScale) {
		case ACCEL_SCALE_2G:
			MPU9250->accMult = 1.0f / 16834.0f;
			break;
		case ACCEL_SCALE_4G:
			MPU9250->accMult = 1.0f / 8192.0f;
			break;
		case ACCEL_SCALE_8G:
			MPU9250->accMult = 1.0f / 4096.0f;
			break;
		case ACCEL_SCALE_16G:
			MPU9250->accMult = 1.0f / 2048.0f;
			break;
	}
	...
	switch (gyroScale) {
		case GYRO_SCALE_250dps:
			MPU9250->gyroMult = 1.0f / 131.0f;
			break;
		case GYRO_SCALE_500dps:
			MPU9250->gyroMult = 1.0f / 65.5f;
			break;
		case GYRO_SCALE_1000dps:
			MPU9250->gyroMult = 1.0f / 32.8f;
			break;
		case GYRO_SCALE_2000dps:
			MPU9250->gyroMult = 1.0f / 16.4f;
			break;
	}
